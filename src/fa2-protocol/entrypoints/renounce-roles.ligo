function renounce_roles (const params : renounce_roles_params; var s : storage) : return is {
  for entry in list params {
    ...
    | Creator -> {
        assert_with_error(is_creator((kt1, Tezos.get_sender()), s), "FA2_INVALID_CREATOR_ACCESS");
      } with permissions with record [ creator = Set.remove(Tezos.get_sender(), permissions.creator) ]
    | Minter -> {
        assert_with_error(is_minter((kt1, Tezos.get_sender()), s), "FA2_INVALID_MINTER_ACCESS");
      } with permissions with record [ minter = Set.remove(Tezos.get_sender(), permissions.minter) ]
    | Metadata_manager -> {
        assert_with_error(is_metadata_manager((kt1, Tezos.get_sender()), s), "FA2_INVALID_METADATA_MANAGER_ACCESS");
      } with permissions with record [ metadata_manager = Set.remove(Tezos.get_sender(), permissions.metadata_manager) ]
    | Royalties_manager -> {
        assert_with_error(is_royalties_manager((kt1, Tezos.get_sender()), s), "FA2_INVALID_ROYALTIES_MANAGER_ACCESS");
