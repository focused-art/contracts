  ...
  for entry in list params {
    ...
    s.hooks[kt1] := case update_hook_param of [
      ...
      | Remove(p) -> {
        ...
        | Transfer_hook -> hooks with record [ transfer = Set.remove(p.1, hooks.transfer) ]
        | Create_hook -> hooks with record [ create = Set.remove(p.1, hooks.create) ]
        | Mint_hook -> hooks with record [ mint = Set.remove(p.1, hooks.mint) ]
        | Burn_hook -> hooks with record [ burn = Set.remove(p.1, hooks.burn) ]
        | Metadata_hook -> hooks with record [ update_metadata = Set.remove(p.1, hooks.update_metadata) ]
      ]
    ];
  };
} with (nil, s)
